...
class RegisterUserForm(forms.ModelForm):
    ...
    def clean_password(self):
        ...
        Usa settings.AUTH_PASSWORD_VALIDATORS para ver si la contraseña se ajusta.
...
class UserEmailUpdateForm(forms.Form):
    ...
    def clean_new_email(self):
        ...
        if (UserModel.objects.filter(email=new_email)
                or RegisterUser.objects.filter(email=new_email)):
            raise forms.ValidationError(ugettext('El email ya existe'))
        return new_email
